...
internal sealed partial class DisableMobileDataCommand : InvokableCommand
{
    ...
    public override ICommandResult Invoke()
    {
        try
        {
            AdbHelper.RunAdb("shell svc data disable", out _, out string error);
            return string.IsNullOrEmpty(error)
                ? AdbSettingsManager.Instance.SuccessToast("Mobile data disabled")
                : ErrorToast($"Failed to disable mobile data: {error}");
        }
