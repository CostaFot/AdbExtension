...
internal sealed partial class LockScreenCommand : InvokableCommand
{
    ...
    public override ICommandResult Invoke()
    {
        try
        {
            AdbHelper.RunAdb("shell input keyevent 26", out _, out string error);
            return string.IsNullOrEmpty(error)
                ? CommandResult.ShowToast(new ToastArgs { Message = "Screen locked", Result = CommandResult.KeepOpen() })
                : ErrorToast($"Failed to lock screen: {error}");
        }
